...
根据 [Seedance 2.5 官方页面](https://seed.bytedance.com/en/seedance2_5)，模型重点面向最长 30 秒的完整叙事，并可继续延长两次；它强化了参考视频理解、音视频编辑、专业运镜、表演调度、白模控制和绿幕编辑。相较于只描述“画面长什么样”，Seedance 2.5 更适合接收一份简洁但完整的导演指令。
...
以下完整示例基于 FLAQ 源库，纸狐狸的开场已按现有配图调整；配图是输入参考，不是视频生成结果。练习尚未在本仓库实测；时长与画幅需按实际平台选项调整。
...
这段首页改写已对应现有配图：狐狸从开头就保持立体纸艺，先转头抬爪，再走下纸页。也可以直接使用[与此图对应的八秒练习](docs/reference-practice.md#fox)。
...
00:00-00:06：立体折纸狐狸轻轻转头，再试探性抬起前爪，折痕、体积和材质始终不变。镜头从速写本左下角微距推近，纸张摩擦声清晰。
00:06-00:13：狐狸跨过纸页边缘踏上真实木桌，保持同一立体折纸结构。每一步的脚部接触连续可见，产生轻微折纸声，桌面倒影和接触阴影准确。
