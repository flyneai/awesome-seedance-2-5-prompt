...
Use the [guided Prompt Submission form](https://github.com/flyneai/awesome-seedance-2-5-prompt/issues/new?template=prompt.yml) for one tested prompt. It asks for the information maintainers need to review the result without requiring you to edit repository files.
...
## Maintaining community video cases

Use the [X video form](https://github.com/flyneai/awesome-seedance-2-5-prompt/issues/new?template=community-video.yml). Follow [source verification](docs/x-showcase-sources.md) and preserve upstream [attribution](docs/PROVENANCE.md).

Edit `docs/x-showcase-sources.json`, run `python3 scripts/build_showcase.py`, then `python3 scripts/check_content.py` before merging. See [maintenance instructions](docs/maintenance.md). Keep status IDs unique and update the README count, Chinese summary, JSON records and changelog together. Check media availability separately using `python3 scripts/check_content.py --check-media`; network failure is not proof that a post was deleted. Do not replace historical check dates until a new check has actually completed. Do not commit keys, cookies, private profiles or downloaded third-party videos.
